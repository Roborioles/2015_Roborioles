...
void PickUpTote::Execute() {
	...
	if(Robot::liftMechanism->upperLimit->Get()&&Robot::liftMechanism->lowerLimit->Get()){
		if(Robot::oi->getGamePad()->GetRawButton(6)){
			if(first){
				Robot::liftMechanism->SetSetpoint(20.8);
				joystick=false;
				first=false;
			}
		} else if(Robot::oi->getGamePad()->GetRawButton(8)){
			if(first){
				Robot::liftMechanism->SetSetpoint(27.8);
				joystick=false;
				first=false;
			}
		} else {
...
// 20.8 - level to hold bin to insert noodle
// 24.3 - level to hold bin to slow incoming totes
// 27.8 - level to hold tote and slow incoming totes
